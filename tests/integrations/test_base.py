...
def event_dto():
    ...
    """
    event_dto = EventDTO(event="test_event", args=("arg1",), kwargs={"key": "value"})
    return event_dto
...
def test_forward_event(eolic: Eolic, event_dto: EventDTO):
    ...
    eolic.listener_handler.emit.assert_called_once_with(
        "test_event", "arg1", key="value"
    )
...
def test_forward_event_without_eolic():
    ...
    with pytest.raises(
        Exception, match="The Eolic instance is not set for the integration"
    ):
        integration.forward_event(event_dto)
