...
def test_fastapi_integration_no_app():
    ...
    """
    with pytest.raises(Exception, match="Please declare you app to setup the integration"):
...
def test_fastapi_empty_route(eolic: Eolic, app: FastAPI):
    ...
    with pytest.raises(Exception, match="Event route is required for FastAPI integration."):
        eolic.setup_integration(integration)
